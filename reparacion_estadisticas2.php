<?php
  ...
  $gastosRepuestosReparacionFinalizada = $db->fetch_assoc($db->query("SELECT SUM(`repuestos`) AS `repuestos`, SUM(`flete`) AS `flete` FROM `reparacion_cotizacion` WHERE `fecha_entrega_maquina` IS NOT NULL"));
  $gastosRepuestosReparacionSinFinalizar = $db->fetch_assoc($db->query("SELECT SUM(`repuestos`) AS `repuestos`, SUM(`flete`) AS `flete` FROM `reparacion_cotizacion` WHERE `fecha_entrega_maquina` IS NULL"));  
  $gastosTotalesFinalizadas = $gastosRepuestosReparacionFinalizada['repuestos'] + $gastosRepuestosReparacionFinalizada['flete'];
  $gastosTotalesSinFinalizar = $gastosRepuestosReparacionSinFinalizar['repuestos'] + $gastosRepuestosReparacionSinFinalizar['flete'];
?>
<div class="row">
					...
					<table id="tbl_marcas-mas-reparadas" class="table table-hover table-condensed table-striped" style="color: rgba(0, 0, 0, 0.8);background-color: white;">
					  ...
					  <?php
					  	...
					  	<tr data-toggle="toolbox" title="Éste repuesto pertenece a la marca <?php echo $unTopRepuesto['marca']; ?>, modelo <?php echo $unTopRepuesto['modelo'];?> <?php echo $unTopRepuesto['descripcion_modelo']; ?>">
					  		<td class="text-center" style="width: 5%;"><?php echo $v."°"; ?></td>
					  		<td style="width: 55%;"><?php echo $unTopRepuesto['descripcion']; ?></td>
					  		<td class="text-center" style="width: 5%;"><?php echo $unTopRepuesto['cantidad_unidades']; ?></td>
					  		<td style="width: 35%;">Unidades</td>
					  	</tr>
					  <?php endforeach; ?>
					  </tbody>
					  <tfoot>
...
<div class="row">
			...
			<div class="inner">
				<div class="row">
					<div class="col-md-5">
						...
						<p style="font-size: 16px;font-weight: 500;margin-top: 15px;width: 100%;text-align: right;">Gastos de repuestos: <span style="font-size: 28px;font-weight: 500;"><?php echo "$ ".$gastosRepuestosReparacionFinalizada['repuestos']; ?></span></p>
						<p style="font-size: 16px;font-weight: 500;margin-top: 5px;width: 100%;text-align: right;">Gastos de fletes: <span style="font-size: 28px;font-weight: 500;"><?php echo "$ ".$gastosRepuestosReparacionFinalizada['flete']; ?></span></p>
					...
					<!------>
					<div class="col-md-5" style="border-left: 1.5px solid rgba(255, 255, 255, 0.7);">
						<p style="font-size: 18px;font-weight: 600;">Gastos Máquinas Sin Entregar</p>
						<hr style="margin-left: 0px;margin-top: -10px;margin-bottom: 8px; width: 100%;">
						<p style="font-size: 16px;font-weight: 500;margin-top: 15px;width: 100%;text-align: right;">Gastos en Repuestos: <span style="font-size: 28px;font-weight: 500;"><?php echo "$ ".$gastosRepuestosReparacionSinFinalizar['repuestos']; ?></span></p>
						<p style="font-size: 16px;font-weight: 500;margin-top: 5px;width: 100%;text-align: right;">Gastos en Fletes: <span style="font-size: 28px;font-weight: 500;"><?php echo "$ ".$gastosRepuestosReparacionSinFinalizar['flete']; ?></span></p>
						<hr style="margin-right: 0px;margin-top: -10px;margin-bottom: 8px; width: 65%;">
	...
	<div class="col-md-4">
		<div class="small-box bg-success" style="height: 220px;/*background-color: rgba(22, 160, 133, 1) !important;*/">
			<div class="inner">
				<p style="font-size: 22px;font-weight: 600;">Ganancias en reparaciones</p>
				<hr style="margin-left: 0px;margin-top: -10px;margin-bottom: 8px; width: 70%;">
				<p style="font-size: 20px;font-weight: 500;margin-top: 15px;width: 70%;text-align: right;">Reparaciones Finalizadas: <span style="font-size: 28px;font-weight: 500;"><?php echo $cantReparacionesFinalizadas; ?></span></p>
				<p style="font-size: 20px;font-weight: 500;margin-top: 5px;width: 70%;text-align: right;">Reparaciones Sin Finalizar: <span style="font-size: 28px;font-weight: 500;"><?php echo $cantReparacionesSinFinalizar; ?></span></p>
				<hr style="margin-left: 5%;margin-top: -10px;margin-bottom: 8px; width: 65%;">
