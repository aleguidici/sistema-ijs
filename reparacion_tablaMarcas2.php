...
<script type="text/javascript">
    ...
    $('#modal_add-modelo').on('hide.bs.modal', function (event) {
    	...
    	$('#sel_tamanio-modelo').val('1');
    	$('#sel_inalambrico').val('0');
	...
	function cargarModelos(refresh) {
		if(!refresh) {
	    ...
	    <?php foreach ($allModelos as $unModelo) : ?>
	      ...
	      var textModelos = "<?php echo $unModelo['codigo'] ;?>";
	    ...
	    <?php endforeach ; ?>
			...
			document.getElementById('inp_id-marca').value = idMarca; //pasa el idMarca al input del boton para relacionar un modelo en caso de crearlo

			console.log('no hay refresh');
		} else { // hace la carga del sel_modelo una vez que se haya creado un modelo nuevo (llama 2 veces la pagina y pasa valores mediante _GET y refresh)
			console.log('hay refresh');
			$('#sel_modelos').find('option').remove().end().append('<option value="" disabled selected>Seleccione un modelox:</option>');
	    	var selModelos = document.getElementById('sel_modelos');
	    <?php 
	    	foreach ($allmodelos as $unModelo) : ?>
	    	if (refresh == <?php echo $unModelo['marca_id']; ?>) {
		      var optModelos = document.createElement('option');
		      var textModelos = "<?php echo $unModelo['codigo'] ;?>";
		      optModelos.appendChild(document.createTextNode(textModelos));
		      optModelos.value = parseInt("<?php echo $unModelo['id']; ?>");
	    ...
	    <?php endforeach ; ?>
	    $('#sel_modelos').selectpicker('refresh');

			
			$('#div_sel-modelos').fadeOut(750);
			$('#div_btn_agregar-modelo').fadeOut(750);
			//var idMarca = document.getElementById("sel_marcas").value;
			alertify.warning("Cargando, por favor espere.");
			$('#contenedor-modelos').fadeOut(750);

			setTimeout(function() {  
			  $('#contenedor-modelos').fadeIn(750);
	      $('#contenedor-modelos').load('reparacion_tablaMarcasDatos.php?id_marca='+ refresh +'');


				//$('#sel_modelos').fadeIn(1000);
				$('#div_sel-modelos').fadeIn(750);
				$('#div_btn_agregar-modelo').fadeIn(750);

				
	    },1250);		
			 // actualiza los valores del selectpicker
	...
	function addModelo() {
		...
		var descripcion = document.getElementById('text_detalle-modelo').value;
		...
		var marca = document.getElementById('inp_id-marca').value;
...
<div class="row">
	<div class="col-md-4">
		<div>
            ...
            <?php
            $getIdMarca = remove_junk($db->escape($_GET['getIdMarca']));
            foreach ($allMarcas as $unaMarca) :
            	if(substr($unaMarca['descripcion'], 0,3) != "---") { 
            		if ($getIdMarca == $unaMarca['id']) { ?>
            			<option value="<?php echo $unaMarca['id']; ?>" selected><?php echo $unaMarca['descripcion']; ?></option>
          <?php } else { ?>
	...
	<div class="col-md-1" style="display: flex; flex-wrap: wrap; align-content:flex-end; padding: 0;"><a type="button" id="a_modal_add-marca" class="btn btn-success" data-toggle="modal" data-target="#modal_add-marca" onclick=""><span class="glyphicon glyphicon-plus" style="color: white;"></span></a>
		...
		<input type="text" name="inp_id-marca" id="inp_id-marca" hidden>
		...
		<script type="text/javascript">
			$(document).on("click", "#a_modal_add-modelo", function () {
				var idMarcaToModal = document.getElementById('inp_id-marca').value;
  ...
  <div class="modal-dialog modal-lg" style="width: 700px !important; margin: 35px auto;" role="document">
    <div class="modal-content">
      ...
      <div class="row">
      	...
      	<div class="col-md-4">
      		<label for="text_detalle-modelo">Descripción: <sup>(Opcional)</sup></label>
        	<textarea type="text" class="form-control" placeholder="Descripción" id="text_detalle-modelo" maxlength="250" style="resize: none; height: 107px;"></textarea>
      	</div>
      </div>     	
      </div>
       <div class="modal-footer">
