<?php
  ...
  $countRepuestos = count($allRepuestosDeEstaReparacion);
...
<script type="text/javascript">
  ...
  $(document).ready(function() {
    // </. hide para btn_editar_total
    document.getElementById('btn_sumar_total').style.display = "none";
...
//borra el proveedor del repuesto
    ...
    function editarProveedorRepuesto(idRepar, idRepue, idProvee, i) {
      document.getElementById('input_precio_'+i).disabled = false;
      document.getElementById('select_iva_'+i).disabled = false;
      document.getElementById("btn_elegir_proveedor_repuesto_"+i).disabled = true;
      document.getElementById("btn_editar_proveedor_repuesto_"+i).style.display = "none";
      document.getElementById("btn_done_editar_proveedor_repuesto_"+i).disabled = false;
      document.getElementById("btn_done_editar_proveedor_repuesto_"+i).style.display = "inline";

      for (var j = 0 ; j < <?php echo $countRepuestos; ?>; j++) {
        if($('#btn_elegir_proveedor_repuesto_'+j).is(":visible")) {
          if ($('#btn_elegir_proveedor_repuesto_'+j).is(":disabled")) {
          } else {
            document.getElementById("btn_elegir_proveedor_repuesto_"+j).disabled = true;
          }
        } else {
          //console.log(j,'not visible');
        }
        if($('#btn_editar_proveedor_repuesto_'+j).is(":visible")) {
          if ($('#btn_editar_proveedor_repuesto_'+j).is(":disabled")) {
          } else {
            document.getElementById('btn_editar_proveedor_repuesto_'+j).disabled = true;
          }
        } else {
          //console.log(j,'not visible');
        }
        if($('#btn_cancelar_proveedor_repuesto_'+j).is(":visible")) {
          if ($('#btn_cancelar_proveedor_repuesto_'+j).is(":disabled")) {
          } else {
            document.getElementById('btn_cancelar_proveedor_repuesto_'+j).disabled = true;
          }
        } else {
          //console.log(j,'not visible');
        }
        if($('#btn_borrar_proveedor_repuesto_'+j).is(":visible")) {
          if ($('#btn_borrar_proveedor_repuesto_'+j).is(":disabled")) {
          } else {
            document.getElementById('btn_borrar_proveedor_repuesto_'+j).disabled = true;
          }
        } else {
          //console.log(j,'not visible');
        }
        if($('#btn_sumar_cantidad_'+j).is(":visible") || $('#btn_restar_cantidad_'+j).is(":visible")) {
          if ($('#btn_sumar_cantidad_'+j).is(":disabled") || $('#btn_restar_cantidad_'+j).is(":disabled")) {
          } else {
            document.getElementById('btn_sumar_cantidad_'+j).disabled = true;
            document.getElementById('btn_restar_cantidad_'+j).disabled = true;
          }
        } else {
          //console.log(j,'not visible');
        }
  ...
  //Elige el proveedor para el set elegido 1 en bd
    ...
    function doneEditarProveedorRepuesto(idRepar, idRepue, idProvee, i) {
      var precio = document.getElementById('input_precio_'+i).value;
      var iva = document.getElementById('select_iva_'+i).value;
      var today = new Date();
      var dd = String(today.getDate()).padStart(2, '0');
      var mm = String(today.getMonth() + 1).padStart(2, '0'); //January is 0!
      var yyyy = today.getFullYear();
      today = dd + '-' + mm + '-' + yyyy;
      if (precio >= 0) {
        done_edit = "&idReparacion=" + idRepar + "&idRepuesto=" + idRepue + "&idProveedor=" + idProvee + "&cantidad=" + 0 + "&precio=" + precio + "&date=" + today + "&elegido=" + 0 + "&idEstado=" + 0 + "&b=" + 8 + "&iva=" + iva ;
        $.ajax({
          type:"POST",
          url:"proveedorAbmSeleccionado.php",
          data:done_edit,
          success:function(r){
            if(r == 1){
              document.getElementById('btn_done_editar_proveedor_repuesto_'+i).disabled = true;
        ...
        alertify.error("Error, el precio no puede ser un número negativo.");
      }   
    }
  ...
  //SET EN BUSQUEDA DE REPUESTO
    ...
    function sumarTotal(idReparacion, sumaRepuestos, senia) {
      document.getElementById('btn_sumar_total').style.display = "none";
      document.getElementById('btn_editar_total').style.display = "inline";
      if (sumaRepuestos >= 0) {
        ...
        var totalFinal = 0 ;
        var montoRecargo = 0 ;
        if (mano_obra != "" && recargo != "" && grasa != "" && flete != "") {
          completo = 1;
        }
        if (recargo <= 1000) {
          if (completo == 1) {
            montoRecargo = ((parseFloat(sumaRepuestos) * parseFloat(recargo)) / 100).toFixed(2) ;
            acumRecargo = (parseFloat(sumaRepuestos) + parseFloat(montoRecargo)).toFixed(2);
            subTotal = (parseFloat(acumRecargo) + parseFloat(grasa) + parseFloat(mano_obra) + parseFloat(flete)).toFixed(2);
            ivaFinal = parseFloat(subTotal * 0.21).toFixed(2) ;
            totalFinal = (parseFloat(subTotal) + parseFloat(ivaFinal) - parseFloat(senia)).toFixed(2) ;
            document.getElementById("input_subtotal").innerHTML = "$ " + subTotal ;
            document.getElementById("input_iva_final").innerHTML = "$ " + ivaFinal ;
            document.getElementById("input_total_final").innerHTML = "$ " + totalFinal ;
            document.getElementById("input_recargo").innerHTML = "$ " + montoRecargo ;
            ...
            send_cotizacion = "&idReparacion=" + idReparacion + "&repuestos=" + sumaRepuestos + "&manoObra=" + mano_obra + "&grasa=" + grasa + "&flete=" + flete + "&recargo=" + recargo + "&acumRecargo=" + montoRecargo + "&senia=" + senia + "&subTotal=" + subTotal + "&ivaFinal=" + ivaFinal + "&totalFinal=" + totalFinal ;
            $.ajax({
              type:"POST",
            ...
            document.getElementById('btn_sumar_total').style.display = "inline";
            document.getElementById('btn_editar_total').style.display = "none";
            alertify.error("Error, hay campos vacios.");
          }
        } else {
          alertify.error("El recargo no puede exceder al 1000%.");
        }
      } else {
        alertify.error("Debe elegir al menos un repuesto.");
      }
    }
/////////////////////////// SUMAR DATOS CON EL BOTON ELEGIR PROVEEDOR
  function sumarMontoCotizacion(idReparacion,sumaRepuestos,senia) {
    if (sumaRepuestos >= 0) {
        var mano_obra = document.getElementById('mano_obra').value;
        var recargo = document.getElementById('recargo').value;
        var grasa = document.getElementById('grasa').value;
        ...
        if (recargo <= 1000) {
          if (completo == 1) {
            ...
            ivaFinal = parseFloat(subTotal * 0.21).toFixed(2) ;
            totalFinal = (parseFloat(subTotal) + parseFloat(ivaFinal) - parseFloat(senia)).toFixed(2) ;
            document.getElementById("input_total_repuestos").innerHTML = "$ " + sumaRepuestos ;
            document.getElementById("input_subtotal").innerHTML = "$ " + subTotal ;
            document.getElementById("input_iva_final").innerHTML = "$ " + ivaFinal ;
            document.getElementById("input_total_final").innerHTML = "$ " + totalFinal ;
            document.getElementById("input_recargo").innerHTML = "$ " + montoRecargo ;
            ...
            $.ajax({
              ...
              success:function(r) {
                if (r == 1) {
                  console.log(idReparacion,sumaTotal,senia);         
                }
              }                    
            });
          } else {
            alertify.error("Error, hay campos vacios en la sección de cotización.");
          }
        } else {
          alertify.error("El recargo no puede exceder al 1000%.");
        }
    ...
    function verPresupuesto(sumaTotal,senia) {
        ...
        if (recargo <= 1000) {
          ...
          document.getElementById("input_subtotal").innerHTML = "$ " + totales ;
          document.getElementById("mano_obra").disabled = true ;
          document.getElementById("recargo").disabled = true ;
          document.getElementById("grasa").disabled = true ;
          document.getElementById("flete").disabled = true ;*/
          window.open('pdf_presupuestoME.php?idReparacion=<?php echo $estaReparacion['id'];?>','_blank');
  ...
  <div class="row">    
    <div class="col-md-12">
      <div class="row">
        ...
        ?>
        <div class="col-md-9">
          <input type="text" name="detalle_nta" hidden id="detalle_nta">
          <br><br><br>
          <div class="pull-left">
      ...
      foreach ($allRepuestosDeEstaReparacion as $unRepuestoDeEstaReparacion) :
        if ($unRepuestoDeEstaReparacion['elegido'] == 1) {
          ...
          if ($unRepuestoDeEstaReparacion['iva'] == 1) {
            $ivaRepuestos = ($tempSuma * 21 ) / 100 ;                    
          } else {
            $ivaRepuestos = 0 ;
          }
          $tempSumaFinal = $tempSuma + $ivaRepuestos ;
          $subTotal = round($subTotal + $tempSumaFinal, 2);
        }        
      endforeach ;
    ?>
    <?php  
    $cotizacionBase = $db->fetch_assoc($db->query("SELECT * FROM `reparacion_cotizacion_base`"));
    $cotizacionReparacion = $db->fetch_assoc($db->query("SELECT * FROM `reparacion_cotizacion` WHERE `reparacion_id` = ".$estaReparacion['id'].""));
    if (!$cotizacionReparacion) {
      $subTotal2 = $cotizacionBase['mano_obra'] + $cotizacionBase['grasa'] + $cotizacionBase['flete'];
      $ivaFinal = round($subTotal2 * 0.21, 2);
      $totalFinal = round($subTotal2 + $ivaFinal - $estaReparacion['senia'], 2);
    } else {
      $ivaFinal = 0 ;
      $totalFinal = 0 ;
      $ivaFinal = round($cotizacionReparacion['subtotal'] * 0.21, 2) ;
      $totalFinal = round($cotizacionReparacion['subtotal'] + $ivaFinal - $estaReparacion['senia'], 2);
    }
    ?>
    ...
    <div class="col-md-12">
      <div class="panel panel-default">
        <div class="panel-heading">
          <div class="row">
            <div class="col-md-10">
              <button hidden id="btn_sumar-cotizacion" onclick="sumarMontoCotizacion('<?php echo $estaReparacion['id']; ?>','<?php echo $subTotal;?>','<?php echo $estaReparacion['senia']; ?>');"></button>
            ...
            <div class="col-md-2" style="vertical-align: middle; text-align: right;">
              <div class="btn-group">
                ...
                <button  type="button" class="btn btn-primary btn-xs" id="btn_sumar_total" name="btn_sumar_total" onclick="javascript:sumarTotal('<?php echo $estaReparacion['id']; ?>','<?php echo $subTotal;?>','<?php echo $estaReparacion['senia']; ?>')" style="width: 35px; height: 35px;"><span class="glyphicon glyphicon-ok"></span></button>
                <?php } else { ?>
                ...
                <button disabled type="button" class="btn btn-primary btn-xs" id="btn_sumar_total" name="btn_sumar_total" onclick="javascript:sumarTotal('<?php echo $estaReparacion['id']; ?>','<?php echo $subTotal;?>','<?php echo $estaReparacion['senia']; ?>')" style="width: 35px; height: 35px;"><span class="glyphicon glyphicon-ok"></span></button>
                <?php } ?>
                <?php if ($cotizacionReparacion) { ?>
                <button type="button" class="btn btn-success btn-xs" id="btn_crear_presupuesto" name="btn_crear_presupuesto" onclick="javascript:verPresupuesto('<?php echo $subTotal;?>','<?php echo $estaReparacion['senia']; ?>')" style="width: 35px; height: 35px;"><span class="glyphicon glyphicon-usd"></span></button>
              <?php } else { ?>
                <button type="button" class="btn btn-success btn-xs" id="btn_crear_presupuesto" name="btn_crear_presupuesto" onclick="javascript:verPresupuesto('<?php echo $subTotal;?>','<?php echo $estaReparacion['senia']; ?>')" disabled style="width: 35px; height: 35px;"><span class="glyphicon glyphicon-usd"></span></button>
              <?php } ?>
                <!--<a type="button" target = '_blank' href="pdf_presupuestoME.php?idReparacion=<?php// echo $estaReparacion['id'];?>&suTo=<?php// echo $subTotal;?>" class="btn btn-success btn-xs" title="Crear Presupuesto" data-toggle="tooltip" id="btn_crear_presupuesto" name="btn_crear_presupuesto" onclick="javascript:pruebaEnviar()"><span class="glyphicon glyphicon-usd"></span></a> -->          
              </div>
            </div>
          </div> 
        </div>
          <div class="table-responsive" style="padding-left: 1px; padding-right: 2px;">
            <table class="table table-bordered table-striped table-hover" style="width: 100%; margin: 0px !important;">
              <thead>
                <tr>
                  <th style="vertical-align:middle; width: 11%;" class="text-center">Repuestos</th>
                  <th colspan="2" style="vertical-align:middle; width: 11%;" class="text-center">Recargo</th>
                  <th style="vertical-align:middle; width: 11%;" class="text-center">Mano de Obra</th>
                  <th style="vertical-align:middle; width: 11%;" class="text-center">Grasa</th>
                  <th style="vertical-align:middle; width: 11%;" class="text-center">Flete</th>                  
                  <th style="vertical-align:middle; width: 11%; color: rgb(220, 20, 60);" class="text-center">Subtotal S/IVA</th>
                  <th style="vertical-align:middle; width: 11%;" class="text-center">Seña</th>
                  
                  <th style="vertical-align:middle; width: 11%; color: rgb(30, 144, 255);" class="text-center">IVA</th>
                  <th style="vertical-align:middle; width: 12%; color: rgb(220, 20, 60);" class="text-center">TOTAL</th>                 
                </tr>
              </thead>
              <tbody>
                <tr>
                  <?php
                    if ($cotizacionReparacion) { ?>                      
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><label name="input_total_repuestos" id="input_total_repuestos"></label></td>
                      <script> document.getElementById('input_total_repuestos').innerHTML = "<?php echo $subTotal; ?>" ; </script></td>
                      <td style="vertical-align:middle; width: 5%;" class="text-center"><input style="width: 70%;" type="number" min="0" max="1000" name="recargo" id="recargo" value="<?php echo $cotizacionReparacion['recargo'];?>" disabled=""><?php echo " %";?></td>
                      <td style="vertical-align:middle; width: 6%;" class="text-center"><label name="input_recago" id="input_recargo"></label></td>
                      <script> document.getElementById('input_recargo').innerHTML = "<?php echo '$ '.round($cotizacionReparacion['acum_recargo'], 2) ;?>" ; </script>
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><?php echo "$ "?><input type="number" name="mano_obra" id="mano_obra" value="<?php echo $cotizacionReparacion['mano_obra'];?>" maxlength="8" disabled></td>
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><?php echo "$ "?><input type="number" min="0" name="grasa" id="grasa" value="<?php echo $cotizacionReparacion['grasa'];?>" maxlength="10" disabled></td>
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><?php echo "$ "?><input type="number" min="0" name="flete" id="flete" value="<?php echo $cotizacionReparacion['flete'];?>" maxlength="10" disabled></td>
                      <td style="vertical-align:middle; width: 11%; color: rgb(220, 20, 60);" class="text-center"><label name="input_subtotal" id="input_subtotal"></label></td>
                      <script> document.getElementById('input_subtotal').innerHTML = "<?php echo '$ '.round($cotizacionReparacion['subtotal'], 2) ;?>" ; </script>
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><label name="input_senia" id="input_senia"></label></td>
                      <script> document.getElementById('input_senia').innerHTML = "<?php echo '$ '.$estaReparacion['senia'] ;?>" ; </script>
                      <td style="vertical-align:middle; width: 8%; color: rgb(30, 144, 255);" class="text-center"><label name="input_iva_final" id="input_iva_final"></label></td>
                      <script> document.getElementById('input_iva_final').innerHTML = "<?php echo '$ '.$ivaFinal ;?>" ; </script>
                      <td style="vertical-align:middle; width: 12%; color: rgb(220, 20, 60);" class="text-center"><label name="input_total_final" id="input_total_final"></label></td>
                      <script> document.getElementById('input_total_final').innerHTML = "<?php echo '$ '.$totalFinal ;?>" ; </script>
                    <?php } else { ?>
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><label name="input_total_repuestos" id="input_total_repuestos"></label></td>
                      <script> document.getElementById('input_total_repuestos').innerHTML = "$ 0" ; </script></td>
                      <td style="vertical-align:middle; width: 5%;" class="text-center"><input style="width: 70%;" type="number" min="0" max="1000" name="recargo" id="recargo" value="<?php echo $cotizacionBase['recargo'];?>" disabled=""><?php echo " %";?></td>
                      <td style="vertical-align:middle; width: 6%;" class="text-center"><label name="input_recago" id="input_recargo"></label></td>
                      <script> document.getElementById('input_recargo').innerHTML = "<?php echo '$ '.round($cotizacionReparacion['acum_recargo'], 2) ;?>" ; </script>
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><?php echo "$ "?><input type="number" name="mano_obra" id="mano_obra" value="<?php echo $cotizacionBase['mano_obra'];?>" maxlength="8" disabled></td>
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><?php echo "$ "?><input type="number" min="0" name="grasa" id="grasa" value="<?php echo $cotizacionBase['grasa'];?>" maxlength="10" disabled></td>
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><?php echo "$ "?><input type="number" min="0" name="flete" id="flete" value="<?php echo $cotizacionBase['flete'];?>" maxlength="10" disabled></td>              
                      <td style="vertical-align:middle; width: 11%; color: rgb(220, 20, 60);" class="text-center"><label name="input_subtotal" id="input_subtotal"></label></td>
                      <script> document.getElementById('input_subtotal').innerHTML = "<?php echo '$ '.$subTotal2; ?>" ; </script>
                      <td style="vertical-align:middle; width: 11%;" class="text-center"><label name="input_senia" id="input_senia"></label></td>
                      <script> document.getElementById('input_senia').innerHTML = "<?php echo '$ '.$estaReparacion['senia']; ?>" ; </script>
                      <td style="vertical-align:middle; width: 8%; color: rgb(30, 144, 255);" class="text-center"><label name="input_iva_final" id="input_iva_final"></label></td>
                      <script> document.getElementById('input_iva_final').innerHTML = "<?php echo '$ '.$ivaFinal; ?>"; </script>
  ...
  ?>
              ...
              <div class="row">
                ...
                <div class="col-md-2" style="text-align: right;">
                  ...
                  <?php if ($allRepuestosDeEstaReparacion[$i]['id_proveedor'] != 1) { 
                    if ($siHayElegido == 1) { ?>  
                    <button id="botonAgregarProveedor_<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Agregar Proveedor" onclick="javascript:agregarProveedorAlRepuesto('<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>')" disabled style="width: 40px;height: 25px;"><span class="glyphicon glyphicon-plus"></span></button>

                    <button id="boton_borrado_<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>" type="submit" class="btn btn-xs btn-danger" data-toggle="tooltip" title="Borrar Repuesto" data-repuestoIdOk="<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>" onclick="javascript:borrarRepuesto('<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['id_proveedor'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['cantidad'];?>')" disabled style="width: 40px;height: 25px;"><span class="glyphicon glyphicon-trash"></span></button>

                  <?php } else { ?>
                    <button id="botonAgregarProveedor_<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Agregar Proveedor" onclick="javascript:agregarProveedorAlRepuesto('<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>')" style="width: 40px;height: 25px;"><span class="glyphicon glyphicon-plus"></span></button>

                    <button id="boton_borrado_<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>" type="submit" class="btn btn-xs btn-danger" data-toggle="tooltip" title="Borrar Repuesto" data-repuestoIdOk="<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>" onclick="javascript:borrarRepuesto('<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['id_proveedor'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['cantidad'];?>')" style="width: 40px;height: 25px;"><span class="glyphicon glyphicon-trash"></span></button> 
              ...
              <table class="table table-bordered table-striped table-hover">
                ...
                <?php  
                  ...
                  if($allRepuestosDeEstaReparacion[$i]['id_proveedor'] != 2){
                    ...
                    <tr>
                    <?php } ?>
                      <td style="vertical-align:middle; width: 25%;"><?php echo $prove['razon_social'];?></td>

                      <td class="text-center" style="vertical-align:middle; width: 10%;">$ <input disabled id="input_precio_<?php echo $i; ?>" type="number" value="<?php echo $allRepuestosDeEstaReparacion[$i]['precio'];?>" min="0" style="width: 85%;"></td>

                      <td class="text-center" style="vertical-align:middle; width: 8%;"><select disabled id="select_iva_<?php echo $i; ?>" style="width: 100%;">
                      <?php 
                      ...
                      <td class="text-center" style="vertical-align:middle; width: 10%;">
                       ...
                       <?php if ($estaReparacion['id_estado'] != 8) { ?> 
                        ...
                        <div class="btn-group">
                          <button disabled id="btn_sumar_cantidad_<?php echo $i; ?>" type="submit" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Agregar" onclick="javascript:sumarCantidad('<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['id_proveedor'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['cantidad'];?>')" >
                            <span class="glyphicon glyphicon-plus"></span>
                          </button>
                          <button disabled id="btn_restar_cantidad_<?php echo $i; ?>" type="submit" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Restar" onclick="javascript:restarCantidad('<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['id_proveedor'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['cantidad'];?>')">
                            <span class="glyphicon glyphicon-minus"></span>
                          </button>
                        </div>
                        <?php } else { ?>
                        <div class="btn-group">
                          <button id="btn_sumar_cantidad_<?php echo $i; ?>" type="submit" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Agregar" onclick="javascript:sumarCantidad('<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['id_proveedor'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['cantidad'];?>')">
                            <span class="glyphicon glyphicon-plus"></span>
                          </button>
                          <button id="btn_restar_cantidad_<?php echo $i; ?>" type="submit" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Restar" onclick="javascript:restarCantidad('<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['id_proveedor'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['cantidad'];?>')">
                            <span class="glyphicon glyphicon-minus"></span>
                          </button>
                        </div>
                        <?php } ?>
                      ...
                      <?php if ($estaReparacion['id_estado'] != 8) {  ?>
                        ...
                        <?php }} else { ?>
                          ...
                          <button id="btn_elegir_proveedor_repuesto_<?php echo $i; ?>" type="submit" class="btn btn-xs btn-success" data-toggle="tooltip" title="Elegir Proveedor" onclick="javascript:elegirProveedorRepuesto('<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['id_proveedor'];?>')" >
                            <span class="glyphicon glyphicon-ok"></span>
                          </button>
                          ...
                          <button disabled style="display: none;" id="btn_done_editar_proveedor_repuesto_<?php echo $i; ?>" type="submit" class="btn btn-xs btn-primary" data-toggle="tooltip" title="Actualizar" onclick="javascript:doneEditarProveedorRepuesto('<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['id_proveedor'];?>','<?php echo $i; ?>')">
                            <span class="glyphicon glyphicon-ok"></span>
                          </button>

                          <button id="btn_borrar_proveedor_repuesto_<?php echo $i; ?>" type="submit" class="btn btn-xs btn-danger" data-toggle="tooltip" title="Borrar Proveedor" onclick="javascript:borrarProveedorRepuesto('<?php echo $allRepuestosDeEstaReparacion[$i]['reparacion_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['repuesto_id'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['id_proveedor'];?>','<?php echo $allRepuestosDeEstaReparacion[$i]['cantidad'];?>')" >
                            <span class="glyphicon glyphicon-trash"></span>
                          </button>
        ...
        window.onload = function() {
          ...
          if (reloading) {
            ...
            sumarMontoCotizacion('<?php echo $estaReparacion['id']; ?>','<?php echo $subTotal;?>','<?php echo $estaReparacion['senia']; ?>');
          }
        }
        </script>     
