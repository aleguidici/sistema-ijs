...
<?php
 if(isset($_POST['abm_proveedor'])){
   ...
   validate_fields($req_field);   
    ...
    if(empty($errors)){
        $query  = "INSERT INTO proveedor (`cuit`, `razon_social`, `direccion`, `localidad`, `provincia`, `cp`, `telefono1`, `telefono2`, `iva`) VALUES ('{$cuit}', '{$razon_social}', '{$direccion}', '{$localidad}', '{$provincia}', '{$cp}', '{$tel1}', '{$tel2}', '{$cond_IVA}')";
        ...
        <form method="post" action="proveedor.php"> 
          ...
          <div class="panel-body">
            <div class="form-group text-right">
              ...
              <button type="submit" name="abm_proveedor" class="btn btn-danger">Crear Proveedor</button>
            </div>
          </div>
        </form> 
      </div>
