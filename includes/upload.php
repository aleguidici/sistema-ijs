...
class Media {
  ...
  public $productPath = SITE_ROOT.DS.'..'.DS.'uploads/imagenes';
 ...
 /*--------------------------------------------------------------*/
  public function process_media($tipo){
    if(!empty($this->errors)){
        return false;
      }
    ...
    if(file_exists($this->productPath."/".$this->fileName)){
      $this->errors[] = "El archivo {$this->fileName} debe ser de tipo '.JPG'.";
      return false;
    }
    ...
    if(move_uploaded_file($this->fileTempPath,$this->productPath.'/'.$this->fileName))
    {
      ...
      $this->errors[] = "Error en la carga de la imagen, posiblemente debido a permisos incorrectos en la carpeta de carga.";
      return false;
    }
...
/*--------------------------------------------------------------*/
  private function insert_media($tipo){

         global $db;
         $sql  = "INSERT INTO imagen ( file_name,file_type, tipo )";
         $sql .=" VALUES ";
         $sql .="(
                  '{$db->escape($this->fileName)}',
                  '{$db->escape($this->fileType)}', '{$db->escape($tipo)}'
                  )";
