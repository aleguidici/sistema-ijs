...
?>
  <?php
  $alModelos = $db->while_loop($db->query("SELECT * FROM `maquina_modelo` WHERE `despiece_id` = 0"));
        ...
        <?php 
          ...
          foreach ($allModelos as $unModelo) :
            if ($unModelo['despiece_id'] != 0) {
              ...
              $tipo = find_by_id('maquina_tipo',$unModelo['marca_id']);
        ?>
          <tr>            
